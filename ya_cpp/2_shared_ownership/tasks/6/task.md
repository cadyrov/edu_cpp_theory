Представьте, что вы устроились в новую компанию и вам доверили обновить приложение для доставки еды из ресторана. 

Из-за того, что в текущем коде, который работает с базой данных SQLite, используются обычные указатели, уже случались большие утечки памяти, приложение зависало и клиенты ресторана уходили к конкурентам.

Скопируйте код ниже к себе и замените в нём все сырые указатели, кроме аргументов функций, на std::unique_ptr и std::out_ptr.
...
- Используйте decltype([](указатель){ код для удаления }) для получения типа лямбды для параметра std::unique_ptr.
- Используйте using, чтобы лаконично именовать умный указатель.
- Используйте std::out_ptr везде, где функция принимает двойной указатель **.
- Удалите все вхождения sqlite3_finalize и sqlite3_close.